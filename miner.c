...
{
	if (work->blk.nonce == NONCE_END ||                // known we are scanning a full nonce range
	    wdiff->tv_sec > opt_scantime ||                 // scan-time has elapsed (user specified, default 60s)
	    work->blk.nonce >= NONCE_END_1 - max_hashes ||   // are there enough nonces left in the work
	    max_hashes >= NONCE_END_1 ||                     // assume we are scanning a full nonce range
...
void allocate_cgpu(struct cgpu_info *cgpu, unsigned int *kp)
{
	...
	for (j = 0; j < threadobj; ++j, ++*kp) {
		...
		thr->_max_nonce = api->can_limit_work ? api->can_limit_work(thr) : NONCE_END;
